...
/**
 * @brief  Calculates exchange matrix "exchange" using Green's functions G(E) = (E - Ham_K)^-1 defined on real frequencies
 * exchange = -1/(2 * S^2 PI) * Im[ \int_{-\inf}^E_F  dE * \Delta_i G^dn_ij * \Delta_j * G^up_ji, \Delta = \Ham^up_K - \Ham^dn_K
 *
